...
// Create an array t hold all the mobile level box in the ui with their given id in the html code
let levelsMobile = [level1m, level2m, level3m, level4m, level5m, level6m, level7m, level8m, level9m, level10m];
...
menuPlayBtn.addEventListener("click", ()=>{
    if (levelSelection == true) {
        ...
        swal.fire(" Please select a level");
    }
...
class EnemyBird {
    ...
    update() {
            ...
            if (this.x + this.width / 2 > canvas.width * 3 + 100) {
                die.play();
                this.x = 0;
                this.y = Math.random() * canvas.height;
            }
...
levelsMobile.forEach((level, levelIndex) => {
    level.addEventListener("click", ()=> {
        gameLevel = levelIndex + 1;
        gameSpeed = gameLevel;
        realSpeed.innerText = gameLevel;
